...
func (s *ChatService) MediaData(ctx context.Context, grant security.Grant, instanceName string, input MediaDataRequest) (MediaDownloadResult, error) {
	started := time.Now()
	...
	messageType, content, keyID, messageID, err := s.resolveMediaDataRequest(ctx, grant, instance.ID, mode, input)
	if err != nil {
		return MediaDownloadResult{}, err
	}
	...
	if err != nil {
		...
		hlog.For(ctx).ErrorContext(ctx, "media download failed",
			"component", "chat_service",
			"error", err,
			"instance_id", instance.ID,
			"instance_name", instance.Name,
			"lookup_mode", string(mode),
			"message_id", messageID,
			"key_id", maskMediaKeyID(keyID),
	...
	if declaredSize > 0 && declaredSize != int64(len(data)) {
		hlog.For(ctx).DebugContext(ctx, "media size differs from declared metadata",
			"component", "chat_service",
			"instance_id", instance.ID,
			"instance_name", instance.Name,
	...
	hlog.For(ctx).DebugContext(ctx, "media downloaded",
		"component", "chat_service",
		"instance_id", instance.ID,
		"instance_name", instance.Name,
		"lookup_mode", string(mode),
...
func (s *ChatService) resolveMediaDataRequest(ctx context.Context, grant security.Grant, instanceID int64, mode MediaDataMode, input MediaDataRequest) (string, json.RawMessage, string, int64, error) {
	switch mode {
	case MediaDataModeID:
		message, err := s.messages.FindByIDForInstance(ctx, grant, instanceID, *input.ID)
		if err != nil {
	...
	case MediaDataModeKeyID:
		message, err := s.messages.FindByKeyIDForInstance(ctx, grant, instanceID, strings.TrimSpace(*input.KeyID))
		if err != nil {
			return "", nil, "", 0, mediaLookupError(err)
		}
		messageType, content, keyID, err := MediaRequestFromStoredMessage(message)
