...
func TestMediaDownloadClientsRejectUnsafeDestinations(t *testing.T) {
	service := NewService(nil, nil, nil, nil, nil)

	_, _, err := service.download(context.Background(), "http://127.0.0.1:1/media", thumbnailMaxBytes)
	if !errors.Is(err, httpclient.ErrInternalAddress) {
		t.Fatalf("media download should reject loopback, got %v", err)
	}
	_, _, _, err = service.downloadAudio(context.Background(), "http://10.0.0.1:1/audio.mp3")
	if !errors.Is(err, httpclient.ErrInternalAddress) {
		t.Fatalf("audio download should reject private address, got %v", err)
	}
	_, _, _, err = service.downloadAudio(context.Background(), "ftp://example.com/audio.mp3")
	if !errors.Is(err, httpclient.ErrUnsupportedScheme) {
		t.Fatalf("audio download should reject unsupported scheme, got %v", err)
	}
}

func TestAudioDownloadUsesNativeResponseLimitAndCallerContext(t *testing.T) {
	type contextKey struct{}
	ctx := context.WithValue(context.Background(), contextKey{}, "request-context")
	contextPreserved := false
	transport := audioRoundTripFunc(func(req *http.Request) (*http.Response, error) {
		contextPreserved = req.Context().Value(contextKey{}) == "request-context"
		return &http.Response{
			StatusCode:    http.StatusOK,
			ContentLength: DefaultAudioConfig().MaxInputBytes + 2,
			Body:          io.NopCloser(strings.NewReader("oversized")),
			Header:        make(http.Header),
		}, nil
	})
	service := NewService(nil, nil, nil, nil, nil)
	service.audioHTTP = newAudioHTTPClient(httpclient.NewFactory(&http.Client{Transport: transport}))

	_, _, _, err := service.downloadAudio(ctx, "https://example.com/audio.mp3")
...
func (f audioRoundTripFunc) RoundTrip(req *http.Request) (*http.Response, error) {
	return f(req)
}
