...
func (s *MessageService) SendMediaFile(ctx context.Context, grant security.Grant, instanceName string, number string, file multipart.File, header *multipart.FileHeader, mediaType string, caption *string, options *MessageOptions) (SendResult, error) {
...
func (s *MessageService) SendContact(ctx context.Context, grant security.Grant, instanceName string, input SendContactRequest) (SendResult, error) {
...
func (s *MessageService) SendLocation(ctx context.Context, grant security.Grant, instanceName string, input SendLocationRequest) (SendResult, error) {
...
func (s *MessageService) SendReaction(ctx context.Context, grant security.Grant, instanceName string, input SendReactionRequest) (SendResult, error) {
	...
	hlog.For(ctx).InfoContext(ctx, "sending WhatsApp reaction",
		"component", "message_service",
		"operation", "message.reaction.send",
		"instance_id", instance.Instance.ID,
		"instance_name", instance.Instance.Name,
		"remote_jid", address.MaskAddress(reaction.RemoteJID.String()),
	)

	id := str.UUID7()
	sendResp, err := managed.Client.SendMessage(ctx, reaction.RemoteJID, protoMessage, whatsmeow.SendRequestExtra{ID: id})
	if err != nil {
		return SendResult{}, fmt.Errorf("%w: %w", ErrSendFailed, err)
	}
	persisted, err := s.persistSentMessage(ctx, grant, instance.Instance, reaction.RemoteJID, string(sendResp.ID), sendResp.Timestamp, reaction.MessageType, reaction.Content, input.Options)
	if err != nil {
		hlog.For(ctx).ErrorContext(ctx, "reaction sent but persistence failed",
			"component", "message_service",
			"error", err,
			"key_id", string(sendResp.ID),
			"instance_id", instance.Instance.ID,
			"key_remote_jid", address.MaskAddress(reaction.RemoteJID.String()),
		)
