...
const (
	maxGroupPictureBytes        = 16 * 1024 * 1024
	groupPictureDownloadTimeout = 20 * time.Second
	maxGroupPictureRedirects    = 5
)
...
type Service interface {
	Create(ctx context.Context, grant security.Grant, instanceName string, input CreateRequest) (InfoResponse, error)
	UpdatePicture(ctx context.Context, grant security.Grant, instanceName string, input UpdatePictureRequest) (InfoResponse, error)
	InviteCode(ctx context.Context, grant security.Grant, instanceName string, groupJID string) (InviteCodeResponse, error)
	RevokeInviteCode(ctx context.Context, grant security.Grant, instanceName string, groupJID string) error
	UpdateParticipant(ctx context.Context, grant security.Grant, instanceName string, groupJID string, input UpdateParticipantRequest) error
	Leave(ctx context.Context, grant security.Grant, instanceName string, groupJID string) error
}
...
func NewService(instances repository.InstanceRepository, clients ConnectedClientResolver) *GroupService {
	return &GroupService{
		instances: instances,
		clients:   clients,
		http:      newGroupHTTPClient(httpclient.NewFactory(nil)),
	}
}

func newGroupHTTPClient(factory *httpclient.Factory) *http.Client {
	if factory == nil {
		factory = httpclient.NewFactory(nil)
	}
	return factory.
		MaxResponseBytes(maxGroupPictureBytes + 1).
		CreatePendingRequest().
		Timeout(groupPictureDownloadTimeout).
		MaxRedirects(maxGroupPictureRedirects).
		CreateClient(nil)
}

func (s *GroupService) Create(ctx context.Context, grant security.Grant, instanceName string, input CreateRequest) (InfoResponse, error) {
	...
	s.logOperation(ctx, instance, "create", info.JID.String())
	return groupInfoResponse(info), nil
}

func (s *GroupService) UpdatePicture(ctx context.Context, grant security.Grant, instanceName string, input UpdatePictureRequest) (InfoResponse, error) {
	...
	s.logOperation(ctx, instance, "update-picture", jid.String())
	return groupInfoResponse(info), nil
}

func (s *GroupService) InviteCode(ctx context.Context, grant security.Grant, instanceName string, groupJID string) (InviteCodeResponse, error) {
	instance, client, jid, err := s.authorizedGroup(ctx, grant, instanceName, groupJID)
	if err != nil {
	...
	s.logOperation(ctx, instance, "invite-code", jid.String())
	return InviteCodeResponse{Invitation: invitation}, nil
}

func (s *GroupService) RevokeInviteCode(ctx context.Context, grant security.Grant, instanceName string, groupJID string) error {
	instance, client, jid, err := s.authorizedGroup(ctx, grant, instanceName, groupJID)
	if err != nil {
		return err
	}
	...
	s.logOperation(ctx, instance, "revoke-invite-code", jid.String())
	return nil
}

func (s *GroupService) UpdateParticipant(ctx context.Context, grant security.Grant, instanceName string, groupJID string, input UpdateParticipantRequest) error {
	...
	s.logOperation(ctx, instance, "update-participant", jid.String())
	return nil
}

func (s *GroupService) Leave(ctx context.Context, grant security.Grant, instanceName string, groupJID string) error {
	instance, client, jid, err := s.authorizedGroup(ctx, grant, instanceName, groupJID)
	if err != nil {
		return err
	}
	...
	s.logOperation(ctx, instance, "leave", jid.String())
	return nil
}

func (s *GroupService) authorizedGroup(ctx context.Context, grant security.Grant, instanceName string, rawGroupJID string) (dbtypes.Instance, *whatsmeow.Client, watypes.JID, error) {
	instance, client, err := s.authorizedClient(ctx, grant, instanceName)
	if err != nil {
...
func (s *GroupService) authorizedClient(ctx context.Context, grant security.Grant, instanceName string) (dbtypes.Instance, *whatsmeow.Client, error) {
...
func (s *GroupService) authenticateInstance(ctx context.Context, grant security.Grant, instanceName string) (dbtypes.Instance, error) {
...
func (s *GroupService) logOperation(ctx context.Context, instance dbtypes.Instance, operation string, groupJID string) {
	hlog.For(ctx).InfoContext(ctx, "WhatsApp group operation completed",
		"component", "group_service",
		"instance_id", instance.ID,
		"instance_name", instance.Name,
		"operation", operation,
		"group_jid", address.MaskAddress(groupJID),
	)
}
