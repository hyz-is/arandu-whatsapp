...
func TestUnhandledWhatsAppEventLogsSafeMetadata(t *testing.T) {
	...
	service.logUnhandledWhatsAppEvent(ctx, &ManagedWhatsAppClient{InstanceID: "42", InstanceName: "beplus"}, &events.PairPasskeyRequest{})

	if records.Len() != 1 {
		t.Fatalf("captured %d records, want 1", records.Len())
	}
	line := records.All()[0]
	if line.Message != "unhandled whatsapp event" ||
		line.Attrs["event_type"] != "*events.PairPasskeyRequest" ||
		line.Attrs["package"] != "go.mau.fi/whatsmeow/types/events" ||
		line.Attrs["instance_id"] != "42" ||
		line.Attrs["instance_name"] != "beplus" {
		t.Fatalf("unexpected log record: %#v", line)
	}
}
