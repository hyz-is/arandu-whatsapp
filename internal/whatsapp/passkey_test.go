...
func TestRequestPasskeyChallengeReturnsStoredChallenge(t *testing.T) {
	...
	result, err := svc.RequestPasskeyChallenge(context.Background(), pairingTestGrant(), "beplus")
	if err != nil {
...
func TestSubmitPasskeyAssertionConsumesBeforeSendAndRejectsRetry(t *testing.T) {
	...
	result, err := svc.SubmitPasskeyAssertion(context.Background(), pairingTestGrant(), "beplus", SubmitPasskeyAssertionRequest{
	...
	_, err = svc.SubmitPasskeyAssertion(context.Background(), pairingTestGrant(), "beplus", SubmitPasskeyAssertionRequest{
		RequestID: challenge.RequestID,
		Assertion: testAssertion(),
	})
...
func TestSubmitPasskeyAssertionRejectsMismatchExpiredAndInvalid(t *testing.T) {
	...
	challenge := session.setPasskeyChallenge(testPublicKey(300000))
	if _, err := svc.SubmitPasskeyAssertion(context.Background(), pairingTestGrant(), "beplus", SubmitPasskeyAssertionRequest{
...
func newPasskeyServiceTest(t *testing.T) (*Service, *pairingSession, *fakePasskeyClient) {
	...
	repo := &fakeInstanceRepository{
		found: dbtypes.InstanceRecord{
			Instance: dbtypes.Instance{ID: 1, Name: "beplus", Status: dbtypes.InstanceStatusOnline},
		},
	}
